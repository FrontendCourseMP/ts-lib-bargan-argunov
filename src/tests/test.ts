        ...
        const validator: FieldValidator = {
          string() {
            if (!['text'].includes(inputField.type)) {
              throw Error("Narrowing of types is impossible because the type 'text' cannot be narrowed to this type.")
            }
          ...
          min(error: string) {
            if (inputField.type === "text") {
              if (inputField.minLength === -1) {
                throw Error(`InputField with name ${inputField.name} does not contain a value for the attribute "minlength"`)
              }
            }
          ...
          max: (error: string) => {
            if (inputField.type === "text") {
              if (inputField.maxLength === -1) {
                throw Error(`InputField with name ${inputField.name} does not contain a value for the attribute "maxlength"`)
              }
            }
            else if(["number", "range"].includes(inputField.type)) {
              if (inputField.max === '') {
